...
function DisplayContainer({
  ...
  waveColors?: SVGWaveColors;
}) {
  return (
    <As
      className={clsx(
        "group relative mx-auto flex drop-shadow-xl xl:w-4/5",
    ...
    >
      <div
        className={cn(
          "bg-card relative -z-[1] mx-auto flex w-[98.6%] flex-col items-center drop-shadow-lg",
          bottom ? "rounded-b-xl" : "rounded-t-xl",
          className,
        )}
      >
        {children}
      ...
      <SVGWave
        className={clsx(bottom ? "-mb-1 -scale-x-100 -scale-y-100" : "-mt-1")}
        {...waveColors}
      />
    </As>
  );
...
export const SVGWave = ({
  ...
  c3,
  ...props
}: SVGProps<SVGSVGElement> & SVGWaveColors) => (
  <svg
    id="Layer_1"
    data-name="Layer 1"
